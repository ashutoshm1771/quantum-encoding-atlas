...
class AmplitudeEncoding(BaseEncoding):
    ...
    **Qubit Ordering Conventions**:
    Different quantum computing frameworks use different conventions for
    qubit ordering when mapping classical data to quantum states. This
    affects how amplitude indices correspond to computational basis states.

    All backends in this implementation use **little-endian ordering**,
    where qubit 0 is the least significant bit (LSB):

    - **PennyLane**: Uses wires=[0, 1, ..., n-1] where wire 0 is LSB.
      The state |x₀x₁...xₙ₋₁⟩ has qubit 0 = x₀ (rightmost in ket notation).
      Amplitude index i maps to basis state with binary representation
      where bit 0 (LSB) is on wire 0.

    - **Qiskit**: Uses qubits [0, 1, ..., n-1] where qubit 0 is LSB.
      Qiskit's initialize() interprets the state vector with qubit 0
      as LSB. The amplitude at index i corresponds to basis state |i⟩
      where the binary representation of i has its LSB on qubit 0.

    - **Cirq**: Uses LineQubit.range(n) where LineQubit(0) is LSB.
      The custom unitary gate prepares states with the same little-endian
      convention, ensuring cross-backend consistency.

    **Example**: For 4 features (2 qubits), amplitude vector [a₀, a₁, a₂, a₃]
    maps to quantum state: a₀|00⟩ + a₁|01⟩ + a₂|10⟩ + a₃|11⟩

    In all backends:
    - Index 0 (binary 00) → qubit 1 = 0, qubit 0 = 0
    - Index 1 (binary 01) → qubit 1 = 0, qubit 0 = 1
    - Index 2 (binary 10) → qubit 1 = 1, qubit 0 = 0
    - Index 3 (binary 11) → qubit 1 = 1, qubit 0 = 1
    ...
    **Hardware Considerations**:
    ...
    def _to_pennylane(self, x: NDArray[np.floating[Any]]) -> Any:
        ...
        Notes
        -----
        **Qubit Ordering**: Uses little-endian convention where wire 0 is
        the least significant bit (LSB). Amplitude index i maps to basis
        state |i⟩ with wire 0 containing bit 0 of the binary representation.

        Example for 2 qubits:
        - amplitudes[0] → |00⟩ (wire 1 = 0, wire 0 = 0)
        - amplitudes[1] → |01⟩ (wire 1 = 0, wire 0 = 1)
        - amplitudes[2] → |10⟩ (wire 1 = 1, wire 0 = 0)
        - amplitudes[3] → |11⟩ (wire 1 = 1, wire 0 = 1)
        """
        try:
    ...
    def _to_qiskit(self, x: NDArray[np.floating[Any]]) -> Any:
        ...
        Notes
        -----
        **Qubit Ordering**: Uses little-endian convention where qubit 0 is
        the least significant bit (LSB). Qiskit's initialize() interprets
        the state vector with this convention, so amplitude index i maps
        to basis state |i⟩ with qubit 0 containing bit 0 of i.

        Example for 2 qubits:
        - x[0] → |00⟩ (qubit 1 = 0, qubit 0 = 0)
        - x[1] → |01⟩ (qubit 1 = 0, qubit 0 = 1)
        - x[2] → |10⟩ (qubit 1 = 1, qubit 0 = 0)
        - x[3] → |11⟩ (qubit 1 = 1, qubit 0 = 1)

        Note that Qiskit's visual circuit diagrams show qubit 0 at the top,
        but measurements and state vectors use LSB-first ordering internally.
        """
        try:
        ...
        #
        # No re-normalization is needed here. Qiskit's initialize() expects
        # a normalized state vector, which we guarantee through our preprocessing.
        # Defensive copy to avoid any potential mutation of caller's data.
        qc.initialize(x.copy(), range(self.n_qubits))
    ...
    def _to_cirq(self, x: NDArray[np.floating[Any]]) -> Any:
        ...
        Notes
        -----
        **Qubit Ordering**: Uses little-endian convention where LineQubit(0)
        is the least significant bit (LSB). The custom unitary gate prepares
        states with amplitude index i mapping to basis state |i⟩ where
        LineQubit(0) contains bit 0 of the binary representation of i.

        Example for 2 qubits:
        - x[0] → |00⟩ (LineQubit(1) = 0, LineQubit(0) = 0)
        - x[1] → |01⟩ (LineQubit(1) = 0, LineQubit(0) = 1)
        - x[2] → |10⟩ (LineQubit(1) = 1, LineQubit(0) = 0)
        - x[3] → |11⟩ (LineQubit(1) = 1, LineQubit(0) = 1)

        This matches the ordering used by PennyLane and Qiskit, ensuring
        cross-backend consistency for amplitude encoding.
